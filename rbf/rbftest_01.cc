...
#include "test_util.h"
        ...
        int RBFTest_private_1(RecordBasedFileManager &rbfm) {
            ...
            if (!equalSizes) {
                ...
                std::cout << "**** [FAIL] RBF Test Private 1 failed. Two files are of different sizes. *****"
                          << std::endl;

                return -1;
            }

            std::cout << "***** RBF Test Case Private 1 Finished. The result will be examined! *****" << std::endl
                      << std::endl;
